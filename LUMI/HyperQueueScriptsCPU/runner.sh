...
timeLimit=48 # to estimate the workload, let's try to complete all jobs within two days
...
for PDBid in ${PDBids[@]}; do
  ...
  then
    ...
    # one pose usually requires 30 min
    timeDemandPose=$(echo ${numberOfPoses} 2 | awk '{print $1/$2}')
...
timeLimit= 12 # to increase our priority in the queue, let's ask for shorter allocations

# start HQ server
mkdir ../HyperQueueRuns/startHQ
cd ../HyperQueueRuns/startHQ
nohup bash ../../HyperQueueScriptsCPU/startHQ.sh >> nohup.out &

echo "Started HQ server"
sleep 10
grep "Pid" nohup.out

cp ../../HyperQueueScriptsCPU/createAllocationsHQ.sh .
...
for PDBid in ${PDBids[@]}; do
  ...
  then
    ...
    mkdir -p ${PDBid}
    cd ${PDBid}
    cp ../../HyperQueueScriptsCPU/runHQ.sh runHQ_${PDBid}.sh
    sed -i runHQ_${PDBid}.sh -e "s/PDBid=/PDBid=${PDBid}/g"
    cp ../../HyperQueueScriptsCPU/hq-md-run.py .
...
nohup bash HyperQueueScriptsCPU/cleanUpSLURM.sh >> nohup.out &
echo $! >> pid.txt
