...
toBeAnalysedFurther=$(for i in ${toBeAnalysedFurther}; do pose=$(head -${i} failed.txt | tail -1); document=$(tail -${NUMBER} HyperQueueRuns/currentlyRunning.txt | grep "${pose::-10}") || echo ${i}; done)
