...
PDBids=($(ls PSWorkflow_experimentalStructures))
...
# start HQ server
mkdir ../HyperQueueRuns_experimentalStructures/startHQ
cd ../HyperQueueRuns_experimentalStructures/startHQ
nohup bash ../../HyperQueueScripts_experimentalStructures/startHQ.sh >> nohup.out &

echo "Started HQ server"
sleep 10
grep "Pid" nohup.out

cp ../../HyperQueueScripts_experimentalStructures/createAllocationsHQ.sh .
...
for PDBid in ${PDBids[@]}; do
  ...
  then
    ...
    mkdir -p ${PDBid}
    cd ${PDBid}
    cp ../../HyperQueueScripts_experimentalStructures/runHQ.sh runHQ_${PDBid}.sh
    sed -i runHQ_${PDBid}.sh -e "s/PDBid=/PDBid=${PDBid}/g"
    cp ../../HyperQueueScripts_experimentalStructures/hq-md-run.py .
...
nohup bash HyperQueueScripts_experimentalStructures/cleanUpSLURM.sh >> nohup.out &
echo $! >> pid.txt
